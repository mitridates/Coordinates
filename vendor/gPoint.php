<?php
namespace App\Coordinates\vendor;
/**
 * PHP class to convert Latitude & Longitude coordinates into UTM & Lambert Conic Conformal Northing/Easting coordinates.
 * 
 * This class encapsulates the methods for representing a geographic point on the earth in three different coordinate systems. Lat/Long, UTM and Lambert Conic Conformal.
 * 
...
//		$y -= 10000000.0;	// Uncomment line to make LOCAL coordinates return southern hemesphere Lat/Long
	...
	public function convertLLtoLCC()
	{
		...
		$lamda	= deg2rad($this->long);						// Longitude to convert
...
//		Alternative formula supposed to be more accurate for sort distances
//		$dist = 2*asin(sqrt( pow(sin(($lat1-$lat2)/2),2) + cos($lat1)*cos($lat2)*pow(sin(($lon1-$lon2)/2),2)));
	 ...
	 * calculates the x,y pixel coordinate that corresponds to the Lat/Long value of
	 ...
	 * version of UTM coordinates that are adjusted to convert the whole area of interest
	 ...
	 * grid that is fully contiguous over the area of the map we are drawing. Similar
	 ...
	 * West of the longitude of Origin will have a negative value in meters. Northing
	...
	public function convertLLtoTM($LongOrigin)
	{
		...
		if (!$LongOrigin) { // Do a standard UTM conversion - so find out what zone the point is in
